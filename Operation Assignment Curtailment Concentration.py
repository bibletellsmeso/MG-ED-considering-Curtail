...
# PV_200 = PV_20 * 10
PV_400 = pd.read_excel('400kW_PV.xlsx')
...
# ---Curtailment---
    ...
    ext.append(curt[0][k][i] <= 60)
    ext.append(curt[1][k][i] >= 0)
    ext.append(curt[1][k][i] <= 30)
        
    ext.append(P_Curt[0][i] == curt[0][0][i])
    ext.append(P_Curt[1][i] == curt[1][0][i])
...
for i in range(24):
    ...
    for j in range(Piece):
        obj += sl1[j] * r[0][j][i] + sl2[j] * r[1][j][i] + sl3[j] * r[2][j][i] + curt[0][0][i] + curt[1][0][i]
...
plt.xlabel('time (h)')
plt.ylabel('Output (kW)')
plt.suptitle("Curtailment 400kW concentrated", fontsize=25)
plt.plot(PV_400, color='green', label='PV_400')
plt.plot(P_Curt[0].value, color='brown', label='Curtail')
plt.plot(PV_400_Output.value, color='blue', label='PV_400 Output')
plt.legend(loc=2)
plt.show
# ---PV2 Curtail
Fig = plt.figure(figsize=[15,10])
plt.xlabel('time (h)')
plt.ylabel('Output (kW)')
plt.suptitle("Curtailment 200kW concentrated", fontsize=25)
